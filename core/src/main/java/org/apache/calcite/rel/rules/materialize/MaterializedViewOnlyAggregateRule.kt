...
@Value.Enclosing
class MaterializedViewOnlyAggregateRule private constructor(config: Config) :
    MaterializedViewAggregateRule<MaterializedViewOnlyAggregateRule.Config?>(config) {
    ...
    interface Config : MaterializedViewAggregateRule.Config {
        @Override
        fun toRule(): MaterializedViewOnlyAggregateRule {
            return MaterializedViewOnlyAggregateRule(this)
        }
        ...
        companion object {
            fun create(relBuilderFactory: RelBuilderFactory?): Config {
                return ImmutableMaterializedViewOnlyAggregateRule.Config.builder()
                    ...
                    .withOperandSupplier { b -> b.operand(Aggregate::class.java).anyInputs() }
