 ...
 *
 *
 * If unrestricted, Delta returns all previous inserts into the table (from
 * time - to now) and all future inserts into the table (from now
 * to +) and never terminates.
 */
abstract class Delta protected constructor(cluster: RelOptCluster?, traits: RelTraitSet?, input: RelNode?) :
    SingleRel(cluster, traits, input) {
    /** Creates a Delta by parsing serialized output.  */
    protected constructor(input: RelInput) : this(input.getCluster(), input.getTraitSet(), input.getInput()) {}
}
