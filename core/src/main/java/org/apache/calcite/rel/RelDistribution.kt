 ...
 * TBD:
 *
 *  * Can we shorten [Type.HASH_DISTRIBUTED] to HASH, etc.
 *  * Do we need [RelDistributions].DEFAULT?
 *  * [RelDistributionTraitDef.convert]
 ...
 *
 */
interface RelDistribution : RelMultipleTrait {
     ...
     *
     *
     * Order is important for some types (RANGE); other types (HASH) consider
     ...
     *
     *
     * For example if relation is HASH distributed by keys [0, 1], after applying
     * a mapping (3, 2, 1, 0), the relation will have a distribution HASH(2,3) because
     * distribution keys changed their ordinals.
     *
     *
     * If mapping eliminates one of the distribution keys, the [Type.ANY]
     * distribution will be returned.
     *
     *
     * If distribution doesn't have keys (BROADCAST or SINGLETON), method will return
     ...
     */
    @Override
    fun apply(mapping: Mappings.TargetMapping?): RelDistribution?
