...
@Value.Enclosing
class MaterializedViewProjectFilterRule private constructor(config: Config) :
    MaterializedViewJoinRule<MaterializedViewProjectFilterRule.Config?>(config) {
    ...
    interface Config : MaterializedViewRule.Config {
        @Override
        fun toRule(): MaterializedViewProjectFilterRule {
            return MaterializedViewProjectFilterRule(this)
        }

        companion object {
            val DEFAULT: Config = ImmutableMaterializedViewProjectFilterRule.Config.builder()
                .withRelBuilderFactory(RelFactories.LOGICAL_BUILDER)
                .withOperandSupplier { b0 ->
                    b0.operand(Project::class.java).oneInput { b1 -> b1.operand(Filter::class.java).anyInputs() }
                }
