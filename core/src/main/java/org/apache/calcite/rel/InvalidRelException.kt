 ...
 * [RelNode] when given parameters it cannot accept. For example,
 * `EnumerableJoinRel` can only implement equi-joins, so its constructor
 * throws `InvalidRelException` when given the condition
 * `input0.x - input1.y = 2`.
 *
 *
 * Because the exception is checked (i.e. extends [Exception] but not
 * [RuntimeException]), constructors that throw this exception will
 * declare this exception in their `throws` clause, and rules that create
 ...
 *
 *
 * The caller that receives an `InvalidRelException` (typically a rule
 * attempting to create a relational expression) should log it at
 * the DEBUG level.
 */
class InvalidRelException : Exception {
    /**
