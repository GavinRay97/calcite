...
enum class NullCollation {
    ...
    fun isDefaultOrder(nullsFirst: Boolean, desc: Boolean): Boolean {
        val asc = !desc
        val nullsLast = !nullsFirst
        return when (this) {
            FIRST -> nullsFirst
            LAST -> nullsLast
            LOW -> asc && nullsFirst || desc && nullsLast
            HIGH -> asc && nullsLast || desc && nullsFirst
            else -> throw IllegalArgumentException("Unrecognized Null Collation")
        }
    }
}
