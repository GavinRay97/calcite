...
import org.apache.calcite.avatica.ConnectionConfigImpl.parse

/**
 * Properties that may be specified on the JDBC connect string.
 */
enum class CalciteConnectionProperty(
    private val camelName: String, type: Type, @Nullable defaultValue: Object?,
    required: Boolean, @Nullable valueClass: Class?
) : ConnectionProperty {
    /** Whether approximate results from `COUNT(DISTINCT ...)` aggregate
     * functions are acceptable.  */
    APPROXIMATE_DISTINCT_COUNT(
        "approximateDistinctCount", Type.BOOLEAN, false,
        false
    ),
    ...
    MATERIALIZATIONS_ENABLED(
        "materializationsEnabled", Type.BOOLEAN, true,
        false
    ),
    ...
    DEFAULT_NULL_COLLATION(
        "defaultNullCollation", Type.ENUM, NullCollation.HIGH,
        true, NullCollation::class.java
    ),
     ...
     * If not specified, value from [.LEX] is used.  */
    QUOTING("quoting", Type.ENUM, null, false, Quoting::class.java),

    /** How identifiers are stored if they are quoted.
     * If not specified, value from [.LEX] is used.  */
    QUOTED_CASING("quotedCasing", Type.ENUM, null, false, Casing::class.java),

    /** How identifiers are stored if they are not quoted.
     * If not specified, value from [.LEX] is used.  */
    UNQUOTED_CASING("unquotedCasing", Type.ENUM, null, false, Casing::class.java),
     ...
     *
     * Ignored if [.MODEL] is specified.  */
    SCHEMA_TYPE("schemaType", Type.ENUM, null, false, JsonSchema.Type::class.java),
     ...
     * [org.apache.calcite.rel.type.RelDataTypeSystem] and has a public
     * default constructor or an `INSTANCE` constant.  */
    TYPE_SYSTEM("typeSystem", Type.PLUGIN, null, false),

    /** SQL conformance level.
     *
     *
     * Controls the semantics of ISO standard SQL features that are implemented
     ...
     * [fun][.FUN] property. If you set `fun=oracle` you will get
    ...
    @Nullable
    private val defaultValue: Object?
    ...
    constructor(
        camelName: String, type: Type, @Nullable defaultValue: Object?,
        required: Boolean
    ...
    init {
        ...
        if (!type.valid(defaultValue, this.valueClass)) {
            throw AssertionError(camelName)
        }
    }
    ...
    @Override
    fun wrap(properties: Properties?): PropEnv {
        return PropEnv(parse(properties, NAME_TO_PROPS), this)
    }

    companion object {
        private val NAME_TO_PROPS: Map<String, CalciteConnectionProperty>? = null

        /** Deprecated; use [.TIME_ZONE].  */
        @Deprecated // to be removed before 2.0
        val TIMEZONE = TIME_ZONE

        init {
            NAME_TO_PROPS = HashMap()
            for (p in values()) {
                NAME_TO_PROPS.put(
                    org.apache.calcite.config.p.camelName.toUpperCase(Locale.ROOT),
                    org.apache.calcite.config.p
                )
                NAME_TO_PROPS.put(org.apache.calcite.config.p.name(), org.apache.calcite.config.p)
            }
        }
    }
}
