...
@JsonSubTypes(
    [JsonSubTypes.Type(
        value = JsonMapSchema::class,
        name = "map"
    ), JsonSubTypes.Type(
        value = JsonJdbcSchema::class,
        name = "jdbc"
    ), JsonSubTypes.Type(value = JsonCustomSchema::class, name = "custom")]
)
abstract class JsonSchema protected constructor(
    /** Name of the schema.
     *
     *
     * Required.
     ...
     */
    val name: String, @Nullable path: List<Object>, @Nullable cache: Boolean?,
    @Nullable autoLattice: Boolean
) {
     ...
     */
    val materializations: List<JsonMaterialization> = ArrayList()
    val lattices: List<JsonLattice> = ArrayList()

    /** Whether to cache metadata (tables, functions and sub-schemas) generated
     * by this schema. Default value is `true`.
     *
     *
     ...
     * case-insensitive
     * (see [org.apache.calcite.config.Lex.caseSensitive]).
     *
     *
     ...
     * other schemas. Default value is `false`.  */
    @Nullable
    val autoLattice: Boolean
    ...
    fun visitChildren(modelHandler: ModelHandler) {
        for (jsonLattice in lattices) {
            jsonLattice.accept(modelHandler)
        }
        for (jsonMaterialization in materializations) {
