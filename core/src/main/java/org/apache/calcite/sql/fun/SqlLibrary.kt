 ...
 * @see LibraryOperator
 */
enum class SqlLibrary(abbrev: String, `fun`: String) {
    ...
    companion object {
        ...
        @Nullable
        fun of(name: String): SqlLibrary? {
            return MAP!![name]
        }

        /** Parses a comma-separated string such as "standard,oracle".  */
        fun parse(libraryNameList: String): List<SqlLibrary> {
            val list: ImmutableList.Builder<SqlLibrary> = ImmutableList.builder()
            for (libraryName in libraryNameList.split(",")) {
                val library: SqlLibrary = Objects.requireNonNull(
                    of(libraryName)
        ...
        val MAP: Map<String, SqlLibrary>? = null

        init {
            val builder: ImmutableMap.Builder<String, SqlLibrary> = ImmutableMap.builder()
            for (value in values()) {
                org.apache.calcite.sql.`fun`.builder.put(
                    org.apache.calcite.sql.`fun`.value.name(),
                    org.apache.calcite.sql.`fun`.value
                )
                org.apache.calcite.sql.`fun`.builder.put(
                    org.apache.calcite.sql.`fun`.value.`fun`,
                    org.apache.calcite.sql.`fun`.value
                )
            }
            MAP = org.apache.calcite.sql.`fun`.builder.build()
        }
    }
}
