 ...
 */
class SqlAdvisorGetHintsFunction2 : TableFunction, ImplementableFunction {
    @get:Override
    ...
    fun getRowType(
        ...
        return typeFactory.createJavaType(SqlAdvisorHint2::class.java)
    }

    @Override
    fun getElementType(arguments: List<Object?>?): Type {
        return SqlAdvisorHint2::class.java
    }
    ...
    companion object {
        ...
        private val GET_COMPLETION_HINTS: Method = Types.lookupMethod(
            SqlAdvisorGetHintsFunction2::class.java, "getCompletionHints",
         ...
         * (via [SqlAdvisorGetHintsFunction2.IMPLEMENTOR]).
         *
        ...
        ): Enumerable<SqlAdvisorHint2> {
            val replaced = arrayOfNulls<String>(1)
            val hints: List<SqlMoniker> = advisor.getCompletionHints(
                sql,
                pos, replaced
            )
            val res: List<SqlAdvisorHint2> = ArrayList(hints.size() + 1)
            res.add(SqlAdvisorHint2(replaced[0], null, "MATCH", null))
            val word = replaced[0]
            for (hint in hints) {
                res.add(SqlAdvisorHint2(hint, advisor.getReplacement(hint, word)))
            }
