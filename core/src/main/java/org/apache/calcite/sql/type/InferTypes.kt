...
object InferTypes {
     ...
     */
    val FIRST_KNOWN: SqlOperandTypeInference = SqlOperandTypeInference { callBinding, returnType, operandTypes ->
        val unknownType: RelDataType = callBinding.getValidator().getUnknownType()
        var knownType: RelDataType = unknownType
        for (operand in callBinding.operands()) {
     ...
     */
    val RETURN_TYPE: SqlOperandTypeInference = SqlOperandTypeInference { callBinding, returnType, operandTypes ->
        for (i in 0 until operandTypes.length) {
            operandTypes.get(i) = if (returnType.isStruct()) returnType.getFieldList().get(i).getType() else returnType
        }
    }
     ...
     * to be boolean.
     */
    val BOOLEAN: SqlOperandTypeInference = SqlOperandTypeInference { callBinding, returnType, operandTypes ->
        val typeFactory: RelDataTypeFactory = callBinding.getTypeFactory()
        for (i in 0 until operandTypes.length) {
            operandTypes.get(i) = typeFactory.createSqlType(SqlTypeName.BOOLEAN)
        }
    }
     ...
     */
    val VARCHAR_1024: SqlOperandTypeInference = SqlOperandTypeInference { callBinding, returnType, operandTypes ->
        val typeFactory: RelDataTypeFactory = callBinding.getTypeFactory()
        for (i in 0 until operandTypes.length) {
            operandTypes.get(i) = typeFactory.createSqlType(SqlTypeName.VARCHAR, 1024)
        }
    }
     ...
     * to be nullable ANY.
     */
    val ANY_NULLABLE: SqlOperandTypeInference = SqlOperandTypeInference { callBinding, returnType, operandTypes ->
        val typeFactory: RelDataTypeFactory = callBinding.getTypeFactory()
        for (i in 0 until operandTypes.length) {
            operandTypes.get(i) = typeFactory.createTypeWithNullability(
                typeFactory.createSqlType(SqlTypeName.ANY), true
            )
        }
    }

    /** Returns an [SqlOperandTypeInference] that returns a given list of
     * types.  */
    fun explicit(types: List<RelDataType?>?): SqlOperandTypeInference {
        return ExplicitOperandTypeInference(ImmutableList.copyOf(types))
    }
}
