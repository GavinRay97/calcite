...
/**
 * Returns the first type that matches a set of given [SqlTypeName]s. If
 * no match could be found, null is returned.
 */
class MatchReturnTypeInference(start: Int, typeNames: Iterable<SqlTypeName?>?) : SqlReturnTypeInference {
    //~ Instance fields --------------------------------------------------------
     ...
     * based).
     *
     * @see .MatchReturnTypeInference
     */
    constructor(start: Int, vararg typeNames: SqlTypeName?) : this(start, ImmutableList.copyOf(typeNames)) {}
    ...
    override fun inferReturnType(
        ...
        for (i in start until opBinding.getOperandCount()) {
            val argType: RelDataType = opBinding.getOperandType(i)
