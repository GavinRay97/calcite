...
/** Creates a ExasolSqlDialect.  */
    ...
    fun supportsAggregateFunction(kind: SqlKind?): Boolean {
        return when (kind) {
            AVG, COUNT, COVAR_POP, COVAR_SAMP, MAX, MIN, STDDEV_POP, STDDEV_SAMP, SUM, VAR_POP, VAR_SAMP -> true
            else -> false
        }
    }
    ...
    fun getCastSpec(type: RelDataType): SqlNode {
        return when (type.getSqlTypeName()) {
            TIMESTAMP ->       // Exasol does not support TIMESTAMP with precision.
                SqlDataTypeSpec(
                    SqlBasicTypeNameSpec(type.getSqlTypeName(), SqlParserPos.ZERO),
                    SqlParserPos.ZERO
                )
    ...
    companion object {
        ...
        val DEFAULT: SqlDialect = ExasolSqlDialect(DEFAULT_CONTEXT)
        ...
        private val RESERVED_KEYWORDS: List<String> = ImmutableList.of(
            "ABS", "ACCESS", "ACOS", "ADAPTER", "ADD_DAYS",
