...
class SqlRankFunction(
    kind: SqlKind, returnTypes: SqlReturnTypeInference?,
    requiresOrder: Boolean
) : SqlAggFunction(
    kind.name(), null, kind, returnTypes, null,
    OperandTypes.NILADIC, SqlFunctionCategory.NUMERIC, requiresOrder,
    true, Optionality.FORBIDDEN
) {
    //~ Constructors -----------------------------------------------------------
    @Deprecated
    constructor(requiresOrder: Boolean, kind: SqlKind) : this(kind, ReturnTypes.INTEGER, requiresOrder) {
    }
