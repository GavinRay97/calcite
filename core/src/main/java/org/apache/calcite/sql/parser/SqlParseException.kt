...
class SqlParseException(
...
) : Exception(message, null), CalciteParserException {
    //~ Instance fields --------------------------------------------------------
     ...
     */
    val expectedTokenSequences: Array<IntArray>?
     ...
     *
     *
     * Also, make it transient, because it is a ParseException generated by
     ...
     *
     *
     * If some of the alternatives contain multiple tokens, returns the last
     ...
     */
    val expectedTokenNames: Collection<String>
        get() {
            if (expectedTokenSequences == null) {
                return Collections.emptyList()
            }
            var maxLength = 0
            for (expectedTokenSequence in expectedTokenSequences) {
                maxLength = Math.max(expectedTokenSequence.size, maxLength)
            }
            val set: Set<String> = TreeSet()
            for (expectedTokenSequence in expectedTokenSequences) {
                if (expectedTokenSequence.size == maxLength) {
                    set.add(
                        tokenImages[expectedTokenSequence[expectedTokenSequence.size - 1]]
                    )
                }
            }
            return set
    ...
    /**
     * Per [java.io.Serializable] API, provides a replacement object to be
