 ...
 *
 * See [Sun's
 * documentation for writers of JDBC drivers](http://java.sun.com/products/jdbc/driverdevs.html).*
 *
 * <table>
 ...
 */
class SqlJdbcFunctionCall(  //~ Instance fields --------------------------------------------------------
    private val jdbcName: String
) : SqlFunction(
    "{fn $jdbcName}",
    SqlKind.JDBC_FN,
    null,
    ...
    @Nullable
    private val lookupMakeCallObj: MakeCall?

    @Nullable
    private var lookupCall: SqlCall?

    @Nullable
    private var thisOperands: @Nullable Array<SqlNode>?

    //~ Constructors -----------------------------------------------------------
    init {
        jdbcName = jdbcName
        lookupMakeCallObj = JdbcToInternalLookupTable.INSTANCE.lookup(
            jdbcName
        )
        lookupCall = null
    }
    ...
    fun getLookupCall(): SqlCall? {
        if (null == lookupCall) {
            lookupCall = requireNonNull(lookupMakeCallObj, "lookupMakeCallObj")
    ...
    fun inferReturnType(
        ...
        val message = lookupMakeCallObj.isValidArgCount(callBinding)
        if (message != null) {
            throw callBinding.newValidationError(
                RESOURCE.wrongNumberOfParam(
                    getName(),
        ...
        val newCall: SqlCall? = getLookupCall()
        val newBinding = SqlCallBinding(
            callBinding.getValidator(), callBinding.getScope(),
            newCall
        )
        val operator: SqlOperator = lookupMakeCallObj.getOperator()
    ...
    private interface MakeCall {
        /**
         * Creates and return a [SqlCall]. If the MakeCall strategy object
         ...
         */
        fun createCall(pos: SqlParserPos?, @Nullable vararg operands: SqlNode?): SqlCall?
        val operator: SqlOperator

        @Nullable
        fun isValidArgCount(binding: SqlCallBinding?): String?
    }

    /** Converter that calls a built-in function with the same arguments.  */
    class SimpleMakeCall(operator: SqlOperator) : MakeCall {
        ...
        @Override
        override fun getOperator(): SqlOperator {
            return operator
        }

        @Override
        override fun createCall(pos: SqlParserPos?, @Nullable vararg operands: SqlNode?): SqlCall {
            return operator.createCall(pos, operands)
        }

        @Override
        @Nullable
        override fun isValidArgCount(binding: SqlCallBinding?): String? {
            return null // any number of arguments is valid
        }
    }

    /** Implementation of [MakeCall] that can re-order or ignore operands.  */
    private class PermutingMakeCall internal constructor(operator: SqlOperator, order: IntArray?) :
        SimpleMakeCall(operator) {
        val order: IntArray
        ...
        @Nullable
        override fun isValidArgCount(binding: SqlCallBinding): String? {
            return if (order.size == binding.getOperandCount()) {
                null // operand count is valid
            } else {
                getArgCountMismatchMsg(order.size)
            }
        }
         ...
         */
        @Nullable
        protected fun reorder(@Nullable operands: Array<SqlNode?>): Array<SqlNode?> {
            assert(operands.size == order.size)
            @Nullable val newOrder: Array<SqlNode?> = arrayOfNulls<SqlNode>(operands.size)
            for (i in operands.indices) {
                assert(operands[i] != null)
                val joyDivision = order[i]
        ...
        companion object {
            private fun getArgCountMismatchMsg(vararg possible: Int): String {
     ...
     */
    private class JdbcToInternalLookupTable @SuppressWarnings("method.invocation.invalid") private constructor() {
        private val map: Map<String, MakeCall>
        ...
        init {
            ...
            val map: ImmutableMap.Builder<String, MakeCall> = ImmutableMap.builder()
            ...
            map.put(
                "INSERT",
                PermutingMakeCall(SqlStdOperatorTable.OVERLAY, intArrayOf(0, 2, 3, 1))
            )
            ...
            map.put("IFNULL",
                object : SimpleMakeCall(SqlStdOperatorTable.COALESCE) {
                    @Override
                    override fun createCall(
                        pos: SqlParserPos?,
                        @Nullable vararg operands: SqlNode?
            ...
            map.put("CONVERT",
                object : SimpleMakeCall(SqlStdOperatorTable.CAST) {
                    @Override
                    override fun createCall(
                        pos: SqlParserPos?,
                        @Nullable vararg operands: SqlNode?
                        ...
                        assert(2 == operands.size)
                        val typeOperand: SqlNode? = operands[1]
                        assert(typeOperand != null && typeOperand.getKind() === SqlKind.LITERAL) { "literal expected, got $typeOperand" }
                        val jdbcType: SqlJdbcDataTypeName = (typeOperand as SqlLiteral?)
                            .getValueAs(SqlJdbcDataTypeName::class.java)
         ...
         */
        @Nullable
        fun lookup(name: String): MakeCall? {
            return map[name]
        }

        companion object {
            /**
             * The [singleton][org.apache.calcite.util.Glossary.SINGLETON_PATTERN]
             * instance.
             */
            val INSTANCE = JdbcToInternalLookupTable()
            private fun trim(flag: SqlTrimFunction.Flag): MakeCall {
                return object : SimpleMakeCall(SqlStdOperatorTable.TRIM) {
                    @Override
                    override fun createCall(
                        pos: SqlParserPos?,
                        @Nullable vararg operands: SqlNode?
            ...
            private fun simple(operator: SqlOperator): MakeCall {
                return SimpleMakeCall(operator)
            }
        }
    }
    ...
    companion object {
        ...
        /**
         * As [java.sql.DatabaseMetaData.getNumericFunctions].
         */
        /** List of all numeric function names defined by JDBC.  */
        val numericFunctions = constructFuncList(
        ...
        /**
         * As [java.sql.DatabaseMetaData.getStringFunctions].
         */
        /** List of all string function names defined by JDBC.  */
        val stringFunctions = constructFuncList(
        ...
        /**
         * As [java.sql.DatabaseMetaData.getTimeDateFunctions].
         */
        /** List of all time/date function names defined by JDBC.  */
        val timeDateFunctions = constructFuncList(
        ...
        /**
         * As [java.sql.DatabaseMetaData.getSystemFunctions].
         */
        /** List of all system function names defined by JDBC.  */
        val systemFunctions = constructFuncList(
            "CONVERT", "DATABASE", "IFNULL", "USER"
        )

        //~ Methods ----------------------------------------------------------------
        private fun constructFuncList(vararg functionNames: String): String {
