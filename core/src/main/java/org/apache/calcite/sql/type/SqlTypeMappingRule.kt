 ...
 *
 *
 * Each instance should define a type mapping matrix which actually defines
...
interface SqlTypeMappingRule {
     ...
     */
    val typeMapping: Map<org.apache.calcite.sql.type.SqlTypeName?, Any?>?

    /** Returns whether it is valid to apply the defined rules from type `from` to
     * type `to`.  */
    fun canApplyFrom(to: SqlTypeName, from: SqlTypeName): Boolean {
        ...
        val rule = typeMapping!![to]
            ?: // If you hit this assert, see the constructor of this class on how
            // to add new rule
            throw AssertionError("No assign rules for $to defined")
