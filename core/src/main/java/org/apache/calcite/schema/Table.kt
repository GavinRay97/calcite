 ...
 * Calcite finds the schema by calling [Schema.getSubSchema] on
 ...
 * [i-node](http://en.wikipedia.org/wiki/Inode) concept in the UNIX
 * filesystem.)
 *
 *
...
interface Table {
     ...
     *
     *
     * This is a struct type whose
     * fields describe the names and types of the columns in this table.
     *
     *
     * The implementer must use the type factory provided. This ensures that
    ...
    val statistic: org.apache.calcite.schema.Statistic?

    /** Type of table.  */
    val jdbcTableType: org.apache.calcite.schema.Schema.TableType?
     ...
     * You can assume that `isRolledUp(column)` is `true`.
     *
     * @param column The column name for which `isRolledUp` is true
     * @param call The aggregate call
     * @param parent Parent node of `call` in the [SqlNode] tree
