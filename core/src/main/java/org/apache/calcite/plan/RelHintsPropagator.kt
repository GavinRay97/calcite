...
/**
 * Customize the propagation of the [org.apache.calcite.rel.hint.RelHint]s
 * from the root relational expression of a rule call [RelOptRuleCall] to
 * the new equivalent expression.
 *
 * @see RelOptUtil.propagateRelHints
 */
@FunctionalInterface
interface RelHintsPropagator {
    /**
     * Propagates the hints from a rule call's root relational expression `oriNode`
     * to the new equivalent relational expression `equiv`.
     *
     ...
     */
    fun propagate(oriNode: RelNode?, equiv: RelNode?): RelNode?
}
