 ...
 */
abstract class RuleQueue protected constructor(planner: VolcanoPlanner) {
    protected val planner: VolcanoPlanner
     ...
     */
    abstract fun addMatch(match: VolcanoRuleMatch?)
     ...
     * [RelNode]s have importance zero.  */
    protected fun skipMatch(match: VolcanoRuleMatch): Boolean {
        for (rel in match.rels) {
        ...
        val subsets: Deque<RelSubset> = ArrayDeque()
        try {
     ...
     */
    private fun checkDuplicateSubsets(
        subsets: Deque<RelSubset>,
        operand: RelOptRuleOperand, rels: Array<RelNode>
    ) {
        val subset: RelSubset = planner.getSubsetNonNull(rels[operand.ordinalInRule])
